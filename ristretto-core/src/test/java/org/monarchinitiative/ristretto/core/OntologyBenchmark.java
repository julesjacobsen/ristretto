...
//    phenol 2.0.3 (pre-CSR implementation)
//    Benchmark                                Mode  Cnt     Score      Error  Units
...
//    OntologyBenchmark.risrettoSuccessors    thrpt    5  3432.519 ±   78.624  ops/s
    ...
    //# VM invoker: /home/hhx640/.jdks/temurin-17.0.11/bin/java
    //# VM options: -javaagent:/home/hhx640/.local/share/JetBrains/Toolbox/apps/intellij-idea-ultimate/lib/idea_rt.jar=35885:/home/hhx640/.local/share/JetBrains/Toolbox/apps/intellij-idea-ultimate/bin -Dfile.encoding=UTF-8
    //# Blackhole mode: full + dont-inline hint
    // phenol 2.1.1 (added CSR implementation)
    // Benchmark                                Mode  Cnt     Score     Error  Units
    //OntologyBenchmark.guavaInDegree         thrpt    5  2384.610 ± 230.956  ops/s
    //OntologyBenchmark.guavaOutDegree        thrpt    5  2595.321 ±  35.493  ops/s
    //OntologyBenchmark.guavaPredecessors     thrpt    5  2418.482 ± 243.225  ops/s
    //OntologyBenchmark.guavaSuccessors       thrpt    5  2324.504 ±  49.335  ops/s
    //OntologyBenchmark.phenolAncestors       thrpt    5    46.003 ±   0.616  ops/s
    //OntologyBenchmark.phenolDescendents     thrpt    5    31.458 ±   0.475  ops/s
    //OntologyBenchmark.phenolInDegree        thrpt    5  1407.695 ±  29.447  ops/s
    //OntologyBenchmark.phenolOutDegree       thrpt    5  1317.551 ±  31.196  ops/s
    //OntologyBenchmark.phenolPredecessors    thrpt    5  3082.744 ±  74.540  ops/s
    //OntologyBenchmark.phenolSuccessors      thrpt    5  2574.603 ±  33.124  ops/s
    //OntologyBenchmark.risrettoAncestors     thrpt    5   108.474 ±   2.866  ops/s
    //OntologyBenchmark.risrettoDescendents   thrpt    5    74.254 ±   1.133  ops/s
    //OntologyBenchmark.risrettoInDegree      thrpt    5  3949.335 ±  42.935  ops/s
    //OntologyBenchmark.risrettoOutDegree     thrpt    5  4116.469 ±  55.191  ops/s
    //OntologyBenchmark.risrettoPredecessors  thrpt    5  3479.795 ±  53.030  ops/s
    //OntologyBenchmark.risrettoSuccessors    thrpt    5  3518.563 ± 979.057  ops/s
    ...
    public void phenolSuccessors(Blackhole blackhole) {
        ...
        hpo.forEach(node -> blackhole.consume(hpo.getChildren(node)));
    }

    @Benchmark
    public void phenolPredecessors(Blackhole blackhole) {
        OntologyGraph<TermId> hpo = phenolHpo;
        hpo.forEach(node -> blackhole.consume(hpo.getParents(node)));
    }

    @Benchmark
    public void phenolInDegree(Blackhole blackhole) {
        OntologyGraph<TermId> hpo = phenolHpo;
        hpo.forEach(node -> blackhole.consume(hpo.getParentsStream(node).count()));
    }

    @Benchmark
    public void phenolOutDegree(Blackhole blackhole) {
        OntologyGraph<TermId> hpo = phenolHpo;
        hpo.forEach(node -> blackhole.consume(hpo.getChildrenStream(node).count()));
    }

    @Benchmark
    public void phenolAncestors(Blackhole blackhole) {
        OntologyGraph<TermId> hpo = phenolHpo;
        hpo.forEach(node -> blackhole.consume(hpo.getAncestorsStream(node).collect(Collectors.toUnmodifiableSet())));
    }

    @Benchmark
    public void phenolDescendents(Blackhole blackhole) {
        OntologyGraph<TermId> hpo = phenolHpo;
        hpo.forEach(node -> blackhole.consume(hpo.getDescendantsStream(node).collect(Collectors.toUnmodifiableSet())));
    }
    ...
    public void risrettoDescendents(Blackhole blackhole) {
        Ontology<Node, Node> hpo = ristrettoHpo;
        for (Node node : hpo.nodes()) {
            blackhole.consume(hpo.descendants(node));
        }
    }
